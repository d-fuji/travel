...
export default function ItineraryImageGallery({
  ...
  const [editingImage, setEditingImage] = useState<ItineraryImage | null>(null);
  ...
  const handleImageEdit = (image: ItineraryImage) => {
    setEditingImage(image);
  };

  const handleImageSave = (updates: Partial<ItineraryImage>) => {
    console.log('Save image updates:', updates);
    // 実際のAPIでは、ここで画像を更新
    if (onImagesChange && editingImage) {
  ...
  return (
    <div className="space-y-3">
      ...
      <ImageViewer
        ...
        onEdit={canEdit ? handleImageEdit : undefined}
